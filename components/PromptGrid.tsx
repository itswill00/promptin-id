        ...
        <div className="mt-3 flex flex-wrap gap-2">
          {CATEGORIES.map((c) => (
            <button
              key={c}
              onClick={() => setCat(c)}
              className={`rounded-full border px-3 py-1 text-sm ${cat === c ? 'bg-black text-white' : 'bg-white'}`}
            >
              {c}
            </button>
          ))}
        ...
        {filtered.map((p) => (
          <article key={p.id} className="relative flex flex-col rounded-xl border bg-white p-4">
            ...
            <div className="mt-3 flex gap-2">
              {p.isPro ? (
                <a href="/pro" className="w-full rounded-lg bg-black px-3 py-2 text-center text-sm text-white">
                  Buka Pro
                </a>
              ) : (
                <button
                  onClick={() => copy(p.id, p.body)}
                  className="w-full rounded-lg border px-3 py-2 text-sm hover:bg-neutral-100"
                >
